...
typedef struct {
	int S;									//状态的数量
	int V;									//噪声变量的数量
	int M;		// only used for parameter estimation	仅用于参数估计
	...
	arm_matrix_instance_f32 Sx;	// state covariance						//状态协方差
	arm_matrix_instance_f32 SxT;// Sx transposed						//状态协方差矩阵的转置
	arm_matrix_instance_f32 Sv;	// process noise						//过程噪声
	arm_matrix_instance_f32 Sn;	// observation noise					//观测噪声
	arm_matrix_instance_f32 x;	// state estimate vector				//状态估计向量
	arm_matrix_instance_f32 Xa;	// augmented sigma points				//增广Sigma矩阵
	float32_t *xIn;
	float32_t *xNoise;
	float32_t *xOut;
	arm_matrix_instance_f32 qrTempS;												//
	arm_matrix_instance_f32 Y;	// resultant measurements from sigma points			//从Sigma点得到的测量量
	arm_matrix_instance_f32 y;	// measurement estimate vector						//测量估计向量
	arm_matrix_instance_f32 qrTempM;
	arm_matrix_instance_f32 Sy;	// measurement covariance							//测量协方差
	arm_matrix_instance_f32 SyT;	// Sy transposed							    //测量协方差矩阵的装置
	arm_matrix_instance_f32 SyC;	// copy of Sy									//测量协方差的副本
	arm_matrix_instance_f32 Pxy;													//状态向量和测量向量之间的协方差
	...
	arm_matrix_instance_f32 xUpdate;												//状态更新矩阵
	arm_matrix_instance_f32 qrFinal;												//最终QR矩阵
