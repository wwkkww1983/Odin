...
// Calculates the QR decomposition of the given matrix A Transposed (decomp's A', not A)		计算给定矩阵转置的QR分解
//      notes:  A matrix is modified		矩阵被修改
//      Adapted from Java code originaly written by Joni Salonen		改编自Joni Salonen写的java代码实现
//
...
// performs Cholesky factorization of 3x3 matrix	//在3x3矩阵的Cholesky分解
int cholF(float32_t *U) {
    ...
    // is positive definite?											//判断是否是正定矩阵
    return (isnan(t4) || isnan(U[8])) ? 0 : 1;		//判断t4和U[8]是否是nan
}
